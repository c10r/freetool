...
type SpaceController
    ...
    member private this.CheckIsSpaceModeratorAsync(spaceId: string) : Task<bool> =
        task {
            let userId = this.CurrentUserId
            let userIdStr = userId.Value.ToString()
    ...
    member private this.CheckIsOrgAdminOrModeratorAsync(spaceId: string) : Task<bool> =
        task {
    ...
    member this.CreateSpace([<FromBody>] createDto: CreateSpaceDto) : Task<IActionResult> =
        task {
            let userId = this.CurrentUserId
            ...
            if not isOrgAdmin then
                ...
                | Ok(SpaceResult spaceData) ->
                    ...
                    return
                        this.CreatedAtAction(nameof this.GetSpaceById, {| id = spaceIdStr |}, spaceData)
                        :> IActionResult
    ...
    [<ProducesResponseType(StatusCodes.Status500InternalServerError)>]
    member this.GetSpaceById(id: string) : Task<IActionResult> =
        task {
    ...
    [<ProducesResponseType(StatusCodes.Status404NotFound)>]
    [<ProducesResponseType(StatusCodes.Status500InternalServerError)>]
    member this.GetSpaceByName(name: string) : Task<IActionResult> =
        task {
    ...
    [<ProducesResponseType(StatusCodes.Status400BadRequest)>]
    [<ProducesResponseType(StatusCodes.Status500InternalServerError)>]
    member this.GetSpaces([<FromQuery>] skip: int, [<FromQuery>] take: int) : Task<IActionResult> =
        task {
    ...
    [<ProducesResponseType(StatusCodes.Status404NotFound)>]
    [<ProducesResponseType(StatusCodes.Status500InternalServerError)>]
    member this.UpdateSpaceName(id: string, [<FromBody>] updateDto: UpdateSpaceNameDto) : Task<IActionResult> =
        task {
            let userId = this.CurrentUserId
            ...
            if not hasPermission then
                logger.LogWarning(
                    "User {UserId} attempted to rename space {SpaceId} without permission",
                    userId.Value,
                    id
                )
    ...
    [<ProducesResponseType(StatusCodes.Status404NotFound)>]
    [<ProducesResponseType(StatusCodes.Status500InternalServerError)>]
    member this.ChangeModerator(id: string, [<FromBody>] changeModeratorDto: ChangeModeratorDto) : Task<IActionResult> =
        task {
            let userId = this.CurrentUserId
    ...
    [<ProducesResponseType(StatusCodes.Status404NotFound)>]
    [<ProducesResponseType(StatusCodes.Status500InternalServerError)>]
    member this.AddMember(id: string, [<FromBody>] addMemberDto: AddMemberDto) : Task<IActionResult> =
        task {
            let userId = this.CurrentUserId
    ...
    [<ProducesResponseType(StatusCodes.Status404NotFound)>]
    [<ProducesResponseType(StatusCodes.Status500InternalServerError)>]
    member this.RemoveMember(id: string, memberId: string) : Task<IActionResult> =
        task {
            let userId = this.CurrentUserId
            ...
            if not hasPermission then
                ...
                return
                    this.Forbidden(
                        "Only organization administrators or space moderators can remove members from spaces"
                    )
            else
    ...
    [<ProducesResponseType(StatusCodes.Status404NotFound)>]
    [<ProducesResponseType(StatusCodes.Status500InternalServerError)>]
    member this.DeleteSpace(id: string) : Task<IActionResult> =
        task {
            let userId = this.CurrentUserId
