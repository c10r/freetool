...
let ``CreateResource - User with create_resource permission can create resources`` () : Task =
    task {
        ...
        let! storeResponse =
            serviceWithoutStore.CreateStoreAsync({ Name = $"test-create-resource-allowed-{storeGuid}" })
...
[<Fact>]
let ``CreateResource - User without create_resource permission cannot create resources`` () : Task =
    task {
        // Arrange - Create store and write model
        let serviceWithoutStore = createServiceWithoutStore ()
        let storeGuid = Guid.NewGuid().ToString("N")

        let! storeResponse = serviceWithoutStore.CreateStoreAsync({ Name = $"test-create-resource-denied-{storeGuid}" })
...
[<Fact>]
let ``CreateResource - Team admin has create_resource permission via admin role`` () : Task =
    task {
        // Arrange - Create store and write model
        let serviceWithoutStore = createServiceWithoutStore ()
        let storeGuid = Guid.NewGuid().ToString("N")

        let! storeResponse = serviceWithoutStore.CreateStoreAsync({ Name = $"test-team-admin-create-{storeGuid}" })
...
[<Fact>]
let ``UpdateResource - User with edit_resource permission can update resources`` () : Task =
    task {
        // Arrange - Create store and write model
        let serviceWithoutStore = createServiceWithoutStore ()
        let storeGuid = Guid.NewGuid().ToString("N")

        let! storeResponse = serviceWithoutStore.CreateStoreAsync({ Name = $"test-edit-resource-allowed-{storeGuid}" })
...
[<Fact>]
let ``UpdateResource - User without edit_resource permission cannot update resources`` () : Task =
    task {
        // Arrange - Create store and write model
        let serviceWithoutStore = createServiceWithoutStore ()
        let storeGuid = Guid.NewGuid().ToString("N")

        let! storeResponse = serviceWithoutStore.CreateStoreAsync({ Name = $"test-edit-resource-denied-{storeGuid}" })
...
[<Fact>]
let ``UpdateResource - Team admin has edit_resource permission via admin role`` () : Task =
    task {
        // Arrange - Create store and write model
        let serviceWithoutStore = createServiceWithoutStore ()
        let storeGuid = Guid.NewGuid().ToString("N")

        let! storeResponse = serviceWithoutStore.CreateStoreAsync({ Name = $"test-team-admin-edit-{storeGuid}" })
...
[<Fact>]
let ``DeleteResource - User with delete_resource permission can delete resources`` () : Task =
    task {
        // Arrange - Create store and write model
        let serviceWithoutStore = createServiceWithoutStore ()
        let storeGuid = Guid.NewGuid().ToString("N")
        ...
        let! storeResponse =
            serviceWithoutStore.CreateStoreAsync({ Name = $"test-delete-resource-allowed-{storeGuid}" })
...
[<Fact>]
let ``DeleteResource - User without delete_resource permission cannot delete resources`` () : Task =
    task {
        // Arrange - Create store and write model
        let serviceWithoutStore = createServiceWithoutStore ()
        let storeGuid = Guid.NewGuid().ToString("N")

        let! storeResponse = serviceWithoutStore.CreateStoreAsync({ Name = $"test-delete-resource-denied-{storeGuid}" })
...
[<Fact>]
let ``DeleteResource - Team admin has delete_resource permission via admin role`` () : Task =
    task {
        // Arrange - Create store and write model
        let serviceWithoutStore = createServiceWithoutStore ()
        let storeGuid = Guid.NewGuid().ToString("N")

        let! storeResponse = serviceWithoutStore.CreateStoreAsync({ Name = $"test-team-admin-delete-{storeGuid}" })
...
[<Fact>]
let ``Global admin has all resource permissions on any workspace`` () : Task =
    task {
        // Arrange - Create store and write model
        let serviceWithoutStore = createServiceWithoutStore ()
        let storeGuid = Guid.NewGuid().ToString("N")

        let! storeResponse = serviceWithoutStore.CreateStoreAsync({ Name = $"test-global-admin-resources-{storeGuid}" })
