    ...
    interface IAuthorizationService with
        ...
        member _.WriteAuthorizationModelAsync() : Task<AuthorizationModelResponse> =
            task {
                ...
                orgMetadata.["admin"] <- RelationMetadata(
                    DirectlyRelatedUserTypes = ResizeArray([
                ...
                // Type: team (with members and admins)
                ...
                teamMetadata.["member"] <- RelationMetadata(
                    DirectlyRelatedUserTypes = ResizeArray([
                        RelationReference(Type = "user")
                    ])
                )
                teamMetadata.["admin"] <- RelationMetadata(
                    DirectlyRelatedUserTypes = ResizeArray([
                        RelationReference(Type = "user")
                ...
                let createPermissionUserset () =
                    ...
                    unionUsersets.Child <- ResizeArray([
                ...
                for permission in ["create_resource"; "edit_resource"; "delete_resource";
                                   "create_app"; "edit_app"; "delete_app"; "run_app"] do
                    workspaceRelations.[permission] <- createPermissionUserset()

                let workspaceMetadata = System.Collections.Generic.Dictionary<string, RelationMetadata>()
                workspaceMetadata.["team"] <- RelationMetadata(
                    DirectlyRelatedUserTypes = ResizeArray([
                        RelationReference(Type = "team")
                    ])
                )

                for permission in ["create_resource"; "edit_resource"; "delete_resource";
                                   "create_app"; "edit_app"; "delete_app"; "run_app"] do
                    workspaceMetadata.[permission] <- RelationMetadata(
                        DirectlyRelatedUserTypes = ResizeArray([
                            RelationReference(Type = "user")
                            RelationReference(Type = "organization", Relation = "admin")
                        ])
                ...
                let body = ClientWriteAuthorizationModelRequest(
                    SchemaVersion = "1.1",
