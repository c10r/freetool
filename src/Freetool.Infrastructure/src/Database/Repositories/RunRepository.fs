...
type RunRepository(context: FreetoolDbContext, eventRepository: IEventRepository) =
    ...
    interface IRunRepository with
        ...
        member _.ExistsAsync(runId: RunId) : Task<bool> =
            task { return! context.Runs.AnyAsync(fun r -> r.Id = runId) }

        member _.GetCountAsync() : Task<int> =
            task { return! context.Runs.CountAsync() }

        member _.GetCountByAppIdAsync(appId: AppId) : Task<int> =
            task { return! context.Runs.CountAsync(fun r -> r.AppId = appId) }

        member _.GetCountByStatusAsync(status: RunStatus) : Task<int> =
            task { return! context.Runs.CountAsync(fun r -> r.Status = status) }

        member _.GetCountByAppIdAndStatusAsync (appId: AppId) (status: RunStatus) : Task<int> =
            task { return! context.Runs.CountAsync(fun r -> r.AppId = appId && r.Status = status) }
