...
type DashboardRepository(context: FreetoolDbContext, eventRepository: IEventRepository) =
    ...
    interface IDashboardRepository with
        member _.GetByIdAsync(dashboardId: DashboardId) : Task<ValidatedDashboard option> =
            task {
        ...
        member _.GetByFolderIdAsync (folderId: FolderId) (skip: int) (take: int) : Task<ValidatedDashboard list> =
            task {
                let! dashboardDatas =
                    context.Dashboards
        ...
        member _.GetBySpaceIdsAsync (spaceIds: SpaceId list) (skip: int) (take: int) : Task<ValidatedDashboard list> =
            task {
        ...
        member _.GetAllAsync (skip: int) (take: int) : Task<ValidatedDashboard list> =
            task {
                let! dashboardDatas =
                    context.Dashboards.OrderBy(fun d -> d.CreatedAt).Skip(skip).Take(take).ToListAsync()

                return dashboardDatas |> Seq.map Dashboard.fromData |> Seq.toList
            }

        member _.AddAsync(dashboard: ValidatedDashboard) : Task<Result<unit, DomainError>> =
            task {
                try
        ...
        member _.UpdateAsync(dashboard: ValidatedDashboard) : Task<Result<unit, DomainError>> =
            task {
                try
        ...
        member _.DeleteAsync (dashboardId: DashboardId) (actorUserId: UserId) : Task<Result<unit, DomainError>> =
            task {
                try
                    let! existingData = context.Dashboards.FirstOrDefaultAsync(fun d -> d.Id = dashboardId)
        ...
        member _.ExistsByNameAndFolderIdAsync (dashboardName: DashboardName) (folderId: FolderId) : Task<bool> =
            task { return! context.Dashboards.AnyAsync(fun d -> d.Name = dashboardName && d.FolderId = folderId) }

        member _.GetCountByFolderIdAsync(folderId: FolderId) : Task<int> =
            task { return! context.Dashboards.CountAsync(fun d -> d.FolderId = folderId) }
        ...
        member _.GetCountAsync() : Task<int> =
            task { return! context.Dashboards.CountAsync() }
