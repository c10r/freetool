...
type UserRepository(context: FreetoolDbContext, eventRepository: IEventRepository) =
    ...
    interface IUserRepository with
        ...
        member _.DeleteAsync(userWithDeleteEvent: ValidatedUser) : Task<Result<unit, DomainError>> =
            task {
                try
        ...
        member _.ExistsAsync(userId: UserId) : Task<bool> =
            task { return! context.Users.AnyAsync(fun u -> u.Id = userId) }
        ...
        member _.GetCountAsync() : Task<int> =
            task { return! context.Users.CountAsync() }
