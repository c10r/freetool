...
type GroupRepository(context: FreetoolDbContext, eventRepository: IEventRepository) =
    ...
    interface IGroupRepository with
        ...
        member _.GetByIdAsync(groupId: GroupId) : Task<ValidatedGroup option> =
            task {
        ...
        member _.GetByNameAsync(name: string) : Task<ValidatedGroup option> =
            task {
        ...
        member _.GetAllAsync (skip: int) (take: int) : Task<ValidatedGroup list> =
            task {
        ...
        member _.GetByUserIdAsync(userId: UserId) : Task<ValidatedGroup list> =
            task {
        ...
        member _.AddAsync(group: ValidatedGroup) : Task<Result<unit, DomainError>> =
            task {
                try
        ...
        member _.UpdateAsync(group: ValidatedGroup) : Task<Result<unit, DomainError>> =
            task {
                try
        ...
        member _.DeleteAsync(groupWithDeleteEvent: ValidatedGroup) : Task<Result<unit, DomainError>> =
            task {
                try
        ...
        member _.ExistsAsync(groupId: GroupId) : Task<bool> =
            task { return! context.Groups.AnyAsync(fun g -> g.Id = groupId) }

        member _.ExistsByNameAsync(name: string) : Task<bool> =
            task { return! context.Groups.AnyAsync(fun g -> g.Name = name) }

        member _.GetCountAsync() : Task<int> =
            task { return! context.Groups.CountAsync() }
