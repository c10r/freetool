...
[<Fact>]
let ``GetEventsByUserIdAsync returns only matching user's events with pagination`` () =
    task {
...
[<Fact>]
let ``GetEventsByAppIdAsync includes app and run events when includeRunEvents is true`` () =
    task {
        use context = createContext ()
        let repository = EventRepository(context) :> IEventRepository
        ...
        Assert.Contains(
            result.Items,
            fun e -> e.EntityType = EntityType.Run && e.EntityId = runIdForApp.Value.ToString()
        )
    }

[<Fact>]
let ``GetEventsByAppIdAsync excludes run events when includeRunEvents is false`` () =
    task {
        use context = createContext ()
        let repository = EventRepository(context) :> IEventRepository
