...
[<Fact>]
let ``CreateSpace returns 403 when user is not org admin`` () : Task =
    task {
        // Arrange
...
[<Fact>]
let ``CreateSpace succeeds when user is org admin`` () : Task =
    task {
        // Arrange
        let userId = UserId.NewId()
...
[<Fact>]
let ``UpdateSpaceName returns 403 when user is not moderator or org admin`` () : Task =
    task {
        // Arrange
        let userId = UserId.NewId()
        let checkPermission _ _ _ = false // No permissions granted
...
[<Fact>]
let ``UpdateSpaceName succeeds when user is moderator`` () : Task =
    task {
        // Arrange
        let userId = UserId.NewId()
...
[<Fact>]
let ``UpdateSpaceName succeeds when user is org admin`` () : Task =
    task {
        // Arrange
        let userId = UserId.NewId()
...
[<Fact>]
let ``AddMember returns 403 when user is neither moderator nor org admin`` () : Task =
    task {
        // Arrange
        let userId = UserId.NewId()
        let checkPermission _ _ _ = false // No permissions granted
        ...
        let controller = createTestController checkPermission handleCommand userId
        let addMemberDto: AddMemberDto = { UserId = UserId.NewId().Value.ToString() }
...
[<Fact>]
let ``AddMember succeeds when user is moderator`` () : Task =
    task {
        // Arrange
        let userId = UserId.NewId()
        let spaceId = "space-123"
...
[<Fact>]
let ``AddMember succeeds when user is org admin`` () : Task =
    task {
        // Arrange
        let userId = UserId.NewId()
...
[<Fact>]
let ``RemoveMember returns 403 when user is neither moderator nor org admin`` () : Task =
    task {
        // Arrange
        let userId = UserId.NewId()
        let checkPermission _ _ _ = false // No permissions granted
...
[<Fact>]
let ``RemoveMember succeeds when user is moderator`` () : Task =
    task {
        // Arrange
        let userId = UserId.NewId()
        let spaceId = "space-123"
...
[<Fact>]
let ``RemoveMember returns 400 when trying to remove moderator`` () : Task =
    task {
        // Arrange
        let userId = UserId.NewId()
        ...
        | :? ObjectResult as objResult when objResult.StatusCode.HasValue ->
            Assert.Equal(422, objResult.StatusCode.Value)
...
[<Fact>]
let ``ChangeModerator returns 403 when user is not org admin`` () : Task =
    task {
        // Arrange
        let userId = UserId.NewId()
...
[<Fact>]
let ``ChangeModerator succeeds when user is org admin`` () : Task =
    task {
        // Arrange
        let userId = UserId.NewId()
...
[<Fact>]
let ``DeleteSpace returns 403 when user is not org admin`` () : Task =
    task {
        // Arrange
        let userId = UserId.NewId()
...
[<Fact>]
let ``GetSpaceById allows any authenticated user`` () : Task =
    task {
        // Arrange
        let userId = UserId.NewId()
        let moderatorId = UserId.NewId()
...
[<Fact>]
let ``GetSpaces allows any authenticated user`` () : Task =
    task {
        // Arrange
        let userId = UserId.NewId()
        let checkPermission _ _ _ = false // No special permissions needed for read
...
[<Fact>]
let ``CreateSpace returns 400 for validation error`` () : Task =
    task {
        // Arrange
        let userId = UserId.NewId()
...
[<Fact>]
let ``GetSpaceById returns 404 for non-existent space`` () : Task =
    task {
        // Arrange
        let userId = UserId.NewId()
...
[<Fact>]
let ``AddMember returns 409 for duplicate member`` () : Task =
    task {
        // Arrange
        let userId = UserId.NewId()
        ...
        let controller = createTestController checkPermission handleCommand userId
        let addMemberDto: AddMemberDto = { UserId = existingMemberId.Value.ToString() }
