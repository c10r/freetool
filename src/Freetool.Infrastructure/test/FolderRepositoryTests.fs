...
[<Fact>]
let ``GetByIdAsync returns folder when persisted FolderData has null Children`` () : Task =
    task {
...
[<Fact>]
let ``GetChildrenAsync initializes Children list for each child when persisted value is null`` () : Task =
    task {
        let context, connection = createSqliteContext ()
        use context = context
        use connection = connection
