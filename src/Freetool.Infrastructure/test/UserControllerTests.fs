...
[<Fact>]
let ``UpdateUserName returns 403 when user is not self and not org admin`` () : Task =
    task {
        // Arrange
...
[<Fact>]
let ``UpdateUserName succeeds when user updates their own profile`` () : Task =
    task {
        // Arrange
        let userId = UserId.NewId()
...
[<Fact>]
let ``UpdateUserName succeeds when user is org admin updating another user`` () : Task =
    task {
        // Arrange
        let userId = UserId.NewId()
...
[<Fact>]
let ``UpdateUserEmail returns 403 when user is not self and not org admin`` () : Task =
    task {
        // Arrange
        let userId = UserId.NewId()
        let targetUserId = UserId.NewId()
...
[<Fact>]
let ``UpdateUserEmail succeeds when user updates their own email`` () : Task =
    task {
        // Arrange
        let userId = UserId.NewId()
        let permissions = Map.empty<AuthSubject * AuthRelation * AuthObject, bool>
...
[<Fact>]
let ``SetProfilePicture returns 403 when user is not self and not org admin`` () : Task =
    task {
        // Arrange
        let userId = UserId.NewId()
        let targetUserId = UserId.NewId()
        ...
        let setDto = { ProfilePicUrl = "https://example.com/pic.jpg" }
...
[<Fact>]
let ``SetProfilePicture succeeds when user updates their own profile picture`` () : Task =
    task {
        // Arrange
        let userId = UserId.NewId()
        let permissions = Map.empty<AuthSubject * AuthRelation * AuthObject, bool>
...
[<Fact>]
let ``RemoveProfilePicture returns 403 when user is not self and not org admin`` () : Task =
    task {
        // Arrange
        let userId = UserId.NewId()
        let targetUserId = UserId.NewId()
...
[<Fact>]
let ``RemoveProfilePicture succeeds when user removes their own profile picture`` () : Task =
    task {
        // Arrange
        let userId = UserId.NewId()
        let permissions = Map.empty<AuthSubject * AuthRelation * AuthObject, bool>
...
[<Fact>]
let ``DeleteUser returns 403 when user is not org admin`` () : Task =
    task {
        // Arrange
        let userId = UserId.NewId()
        let targetUserId = UserId.NewId()
...
[<Fact>]
let ``DeleteUser succeeds when user is org admin`` () : Task =
    task {
        // Arrange
        let userId = UserId.NewId()
        let targetUserId = UserId.NewId()
...
[<Fact>]
let ``DeleteUser returns 403 even when user tries to delete themselves`` () : Task =
    task {
        // Arrange
        let userId = UserId.NewId()
...
[<Fact>]
let ``GetUserById allows any authenticated user`` () : Task =
    task {
        // Arrange
        let userId = UserId.NewId()
        let targetUserId = UserId.NewId()
...
[<Fact>]
let ``GetUserByEmail allows any authenticated user`` () : Task =
    task {
        // Arrange
        let userId = UserId.NewId()
...
[<Fact>]
let ``GetUsers allows any authenticated user`` () : Task =
    task {
        // Arrange
        let userId = UserId.NewId()
