...
[<Fact>]
let ``Space moderator should have all space permissions`` () : Task =
    task {
        // Arrange - Create store, write model
        let serviceWithoutStore = createServiceWithoutStore ()
        let! storeResponse = serviceWithoutStore.CreateStoreAsync({ Name = $"test-space-moderator-{Guid.NewGuid()}" })
...
[<Fact>]
let ``Space member should NOT have permissions by default`` () : Task =
    task {
        // Arrange - Create store, write model
        let serviceWithoutStore = createServiceWithoutStore ()

        let! storeResponse =
            serviceWithoutStore.CreateStoreAsync({ Name = $"test-space-member-no-perms-{Guid.NewGuid()}" })
...
[<Fact>]
let ``Organization admin should have all permissions on any space`` () : Task =
    task {
        // Arrange - Create store, write model
        let serviceWithoutStore = createServiceWithoutStore ()
        let! storeResponse = serviceWithoutStore.CreateStoreAsync({ Name = $"test-org-admin-space-{Guid.NewGuid()}" })
...
[<Fact>]
let ``Only org admin can delete spaces`` () : Task =
    task {
        // Arrange - Create store, write model
        let serviceWithoutStore = createServiceWithoutStore ()
        let! storeResponse = serviceWithoutStore.CreateStoreAsync({ Name = $"test-space-delete-{Guid.NewGuid()}" })
...
[<Fact>]
let ``Only org admin can create spaces`` () : Task =
    task {
        // Arrange - Create store, write model
        let serviceWithoutStore = createServiceWithoutStore ()
        let! storeResponse = serviceWithoutStore.CreateStoreAsync({ Name = $"test-space-create-{Guid.NewGuid()}" })
...
[<Fact>]
let ``Moderator can rename their space`` () : Task =
    task {
        // Arrange - Create store, write model
        let serviceWithoutStore = createServiceWithoutStore ()
        let! storeResponse = serviceWithoutStore.CreateStoreAsync({ Name = $"test-space-rename-{Guid.NewGuid()}" })
...
[<Fact>]
let ``Member cannot rename space`` () : Task =
    task {
        // Arrange - Create store, write model
        let serviceWithoutStore = createServiceWithoutStore ()

        let! storeResponse =
            serviceWithoutStore.CreateStoreAsync({ Name = $"test-space-member-rename-{Guid.NewGuid()}" })
...
[<Fact>]
let ``Moderator can add members to space`` () : Task =
    task {
        // Arrange - Create store, write model
        let serviceWithoutStore = createServiceWithoutStore ()
        let! storeResponse = serviceWithoutStore.CreateStoreAsync({ Name = $"test-space-add-member-{Guid.NewGuid()}" })
...
[<Fact>]
let ``Moderator can remove members from space`` () : Task =
    task {
        // Arrange - Create store, write model
        let serviceWithoutStore = createServiceWithoutStore ()

        let! storeResponse =
            serviceWithoutStore.CreateStoreAsync({ Name = $"test-space-remove-member-{Guid.NewGuid()}" })
...
[<Fact>]
let ``Member cannot add or remove other members`` () : Task =
    task {
        // Arrange - Create store, write model
        let serviceWithoutStore = createServiceWithoutStore ()
        let! storeResponse = serviceWithoutStore.CreateStoreAsync({ Name = $"test-space-member-mgmt-{Guid.NewGuid()}" })
...
[<Fact>]
let ``Space member with granted permission can access resource`` () : Task =
    task {
        // Arrange - Create store, write model
        let serviceWithoutStore = createServiceWithoutStore ()
...
[<Fact>]
let ``Org admin inherits space permissions via tupleToUserset`` () : Task =
    task {
        // Arrange - Create store, write model
        let serviceWithoutStore = createServiceWithoutStore ()
        let! storeResponse = serviceWithoutStore.CreateStoreAsync({ Name = $"test-org-inherit-space-{Guid.NewGuid()}" })
...
[<Fact>]
let ``Moderator of one space cannot access another space`` () : Task =
    task {
        // Arrange - Create store, write model
        let serviceWithoutStore = createServiceWithoutStore ()
        let! storeResponse = serviceWithoutStore.CreateStoreAsync({ Name = $"test-space-isolation-{Guid.NewGuid()}" })
