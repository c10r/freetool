...
[<Fact>]
let ``CreateUser succeeds with valid user`` () =
    task {
        // Arrange
...
[<Fact>]
let ``CreateUser fails when email exists`` () =
    task {
        // Arrange
        let actorUserId = UserId.NewId()
...
[<Fact>]
let ``DeleteUser removes user from repository`` () =
    task {
        // Arrange
        let actorUserId = UserId.NewId()
...
[<Fact>]
let ``DeleteUser returns NotFound for nonexistent user`` () =
    task {
        // Arrange
        let actorUserId = UserId.NewId()
...
[<Fact>]
let ``UpdateUserName succeeds for existing user`` () =
    task {
        // Arrange
        let actorUserId = UserId.NewId()
...
[<Fact>]
let ``UpdateUserName returns NotFound for nonexistent user`` () =
    task {
        // Arrange
        let actorUserId = UserId.NewId()
        let nonExistentUserId = Guid.NewGuid().ToString()
...
[<Fact>]
let ``UpdateUserEmail succeeds for existing user`` () =
    task {
        // Arrange
        let actorUserId = UserId.NewId()
...
[<Fact>]
let ``UpdateUserEmail returns ValidationError for invalid email`` () =
    task {
        // Arrange
        let actorUserId = UserId.NewId()
...
[<Fact>]
let ``SetProfilePicture updates user profile pic`` () =
    task {
        // Arrange
        let actorUserId = UserId.NewId()
...
[<Fact>]
let ``RemoveProfilePicture clears profile pic`` () =
    task {
        // Arrange
        let actorUserId = UserId.NewId()
...
[<Fact>]
let ``GetUserById returns user when exists`` () =
    task {
        // Arrange
        let existingUser = createTestUser "Test User" "user@example.com"
        let userId = User.getId existingUser
...
[<Fact>]
let ``GetUserByEmail returns user when exists`` () =
    task {
        // Arrange
...
[<Fact>]
let ``GetAllUsers returns paginated results`` () =
    task {
        // Arrange
        let users =
...
[<Fact>]
let ``InviteUser creates placeholder user`` () =
    task {
        // Arrange
        let actorUserId = UserId.NewId()
