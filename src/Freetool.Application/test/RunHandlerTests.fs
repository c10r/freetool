...
type MockRunRepository(runs: ValidatedRun list) =
    ...
    interface IRunRepository with
        member _.GetByIdAsync(runId: RunId) : Task<ValidatedRun option> =
            task { return runList |> List.tryFind (fun r -> r.State.Id = runId) }

        member _.GetByAppIdAsync (appId: AppId) (skip: int) (take: int) : Task<ValidatedRun list> =
            task {
                return
                    runList
        ...
        member _.GetByStatusAsync (status: RunStatus) (skip: int) (take: int) : Task<ValidatedRun list> =
            task {
                return
                    runList
        ...
        member _.AddAsync(run: ValidatedRun) : Task<Result<unit, DomainError>> =
            task {
                runList <- run :: runList
                return Ok()
            }

        member _.UpdateAsync(run: ValidatedRun) : Task<Result<unit, DomainError>> =
            task {
                runList <- run :: (runList |> List.filter (fun r -> r.State.Id <> run.State.Id))
                return Ok()
            }

        member _.ExistsAsync(runId: RunId) : Task<bool> =
            task { return runList |> List.exists (fun r -> r.State.Id = runId) }

        member _.GetCountAsync() : Task<int> = task { return runList.Length }

        member _.GetCountByAppIdAsync(appId: AppId) : Task<int> =
            task { return runList |> List.filter (fun r -> r.State.AppId = appId) |> List.length }

        member _.GetCountByStatusAsync(status: RunStatus) : Task<int> =
            task { return runList |> List.filter (fun r -> r.State.Status = status) |> List.length }

        member _.GetCountByAppIdAndStatusAsync (appId: AppId) (status: RunStatus) : Task<int> =
            task {
                return
                    runList
                    |> List.filter (fun r -> r.State.AppId = appId && r.State.Status = status)
...
[<Fact>]
let ``CreateRun succeeds with valid app and inputs`` () =
    task {
        // Arrange
...
[<Fact>]
let ``CreateRun fails for nonexistent app`` () =
    task {
        // Arrange
        let runRepository = MockRunRepository([]) :> IRunRepository
...
[<Fact>]
let ``CreateRun fails when resource not found`` () =
    task {
        // Arrange
        let folderId = FolderId.NewId()
...
[<Fact>]
let ``CreateRun marks as invalid when input validation fails`` () =
    task {
        // Arrange
        let spaceId = SpaceId.NewId()
        let folderId = FolderId.NewId()
...
[<Fact>]
let ``GetRunById returns run when exists`` () =
    task {
        // Arrange
...
[<Fact>]
let ``GetRunById returns NotFound for nonexistent run`` () =
    task {
        // Arrange
        let runRepository = MockRunRepository([]) :> IRunRepository
        let appRepository = MockAppRepository([]) :> IAppRepository
...
[<Fact>]
let ``GetRunsByAppId returns paginated runs`` () =
    task {
        // Arrange
        let appId = AppId.NewId()
...
[<Fact>]
let ``GetRunsByStatus returns runs with matching status`` () =
    task {
        // Arrange
...
[<Fact>]
let ``GetRunsByStatus fails for invalid status string`` () =
    task {
        // Arrange
        let runRepository = MockRunRepository([]) :> IRunRepository
        let appRepository = MockAppRepository([]) :> IAppRepository
...
[<Fact>]
let ``GetRunsByAppIdAndStatus combines filters`` () =
    task {
        // Arrange
        let appId1 = AppId.NewId()
        let appId2 = AppId.NewId()
...
[<Fact>]
let ``GetRunsByAppIdAndStatus with invalid app ID returns validation error`` () =
    task {
        // Arrange
        let runRepository = MockRunRepository([]) :> IRunRepository
        let appRepository = MockAppRepository([]) :> IAppRepository
