...
type MockUserRepository(users: ValidatedUser list) =
    ...
    interface IUserRepository with
        member _.GetByIdAsync(userId: UserId) : Task<ValidatedUser option> =
            task { return userList |> List.tryFind (fun u -> u.State.Id = userId) }

        member _.GetByEmailAsync(email: Email) : Task<ValidatedUser option> =
            task { return userList |> List.tryFind (fun u -> u.State.Email = email.Value) }

        member _.GetAllAsync (skip: int) (take: int) : Task<ValidatedUser list> =
            task { return userList |> List.skip skip |> List.truncate take }
        ...
        member _.DeleteAsync(user: ValidatedUser) : Task<Result<unit, DomainError>> =
            task {
                userList <- userList |> List.filter (fun u -> u.State.Id <> user.State.Id)
                return Ok()
            }

        member _.ExistsAsync(userId: UserId) : Task<bool> =
            task { return userList |> List.exists (fun u -> u.State.Id = userId) }

        member _.ExistsByEmailAsync(email: Email) : Task<bool> =
            task { return userList |> List.exists (fun u -> u.State.Email = email.Value) }
...
type MockSpaceRepository(spaces: ValidatedSpace list) =
    ...
    interface ISpaceRepository with
        member _.GetByIdAsync(spaceId: SpaceId) : Task<ValidatedSpace option> =
            task { return spaceList |> List.tryFind (fun s -> s.State.Id = spaceId) }

        member _.GetByNameAsync(name: string) : Task<ValidatedSpace option> =
            task { return spaceList |> List.tryFind (fun s -> s.State.Name = name) }

        member _.GetAllAsync (skip: int) (take: int) : Task<ValidatedSpace list> =
            task { return spaceList |> List.skip skip |> List.truncate take }

        member _.GetByUserIdAsync(userId: UserId) : Task<ValidatedSpace list> =
            task { return spaceList |> List.filter (fun s -> List.contains userId s.State.MemberIds) }

        member _.GetByModeratorUserIdAsync(userId: UserId) : Task<ValidatedSpace list> =
            task { return spaceList |> List.filter (fun s -> s.State.ModeratorUserId = userId) }
        ...
        member _.DeleteAsync(space: ValidatedSpace) : Task<Result<unit, DomainError>> =
            task {
        ...
        member _.ExistsAsync(spaceId: SpaceId) : Task<bool> =
            task { return spaceList |> List.exists (fun s -> s.State.Id = spaceId) }

        member _.ExistsByNameAsync(name: string) : Task<bool> =
            task { return spaceList |> List.exists (fun s -> s.State.Name = name) }
...
type MockAuthorizationService() =
    ...
    interface IAuthorizationService with
        member _.CreateStoreAsync(_request: CreateStoreRequest) : Task<StoreResponse> =
            task { return { Id = "test-store"; Name = "test" } }

        member _.WriteAuthorizationModelAsync() : Task<AuthorizationModelResponse> =
            task { return { AuthorizationModelId = "test-model" } }

        member _.InitializeOrganizationAsync (_organizationId: string) (_adminUserId: string) : Task<unit> =
            task { return () }
        ...
        member _.CheckPermissionAsync (subject: AuthSubject) (relation: AuthRelation) (obj: AuthObject) : Task<bool> =
            task {
                let key =
                    sprintf
                        "%s-%s-%s"
...
[<Fact>]
let ``CreateSpace succeeds with valid data`` () =
    task {
        // Arrange
...
[<Fact>]
let ``CreateSpace fails when name already exists`` () =
    task {
        // Arrange
        let moderator = createTestUser "Moderator User" "moderator@test.com"
...
[<Fact>]
let ``CreateSpace fails when moderator not found`` () =
    task {
        // Arrange
...
[<Fact>]
let ``CreateSpace fails when member not found`` () =
    task {
        // Arrange
        let moderator = createTestUser "Moderator User" "moderator@test.com"
...
[<Fact>]
let ``UpdateSpaceName changes name`` () =
    task {
        // Arrange
        let moderator = createTestUser "Moderator User" "moderator@test.com"
...
[<Fact>]
let ``UpdateSpaceName fails for duplicate name`` () =
    task {
        // Arrange
        let moderator = createTestUser "Moderator User" "moderator@test.com"
...
[<Fact>]
let ``ChangeModerator updates moderator`` () =
    task {
        // Arrange
...
[<Fact>]
let ``ChangeModerator fails when user not found`` () =
    task {
        // Arrange
        let originalModerator = createTestUser "Original Moderator" "original@test.com"
...
[<Fact>]
let ``AddMember adds user to space`` () =
    task {
        // Arrange
        let moderator = createTestUser "Moderator User" "moderator@test.com"
...
[<Fact>]
let ``AddMember fails when user not found`` () =
    task {
        // Arrange
        let moderator = createTestUser "Moderator User" "moderator@test.com"
        let nonExistentUserId = UserId.NewId()
...
[<Fact>]
let ``RemoveMember removes user from space`` () =
    task {
        // Arrange
        let moderator = createTestUser "Moderator User" "moderator@test.com"
...
[<Fact>]
let ``GetSpaceById returns space when exists`` () =
    task {
        // Arrange
        let moderator = createTestUser "Moderator User" "moderator@test.com"
        let space = createTestSpace "Test Space" moderator.State.Id moderator.State.Id None
...
[<Fact>]
let ``GetSpacesByUserId returns user spaces`` () =
    task {
        // Arrange
...
[<Fact>]
let ``GetSpaceMembersWithPermissions returns permissions`` () =
    task {
        // Arrange
        let moderator = createTestUser "Moderator User" "moderator@test.com"
