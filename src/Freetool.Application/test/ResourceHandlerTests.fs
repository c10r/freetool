...
type MockResourceRepository(resources: ValidatedResource list) =
    ...
    interface IResourceRepository with
        ...
        member _.GetAllAsync (skip: int) (take: int) : Task<ValidatedResource list> =
            task { return resourceList |> List.skip skip |> List.truncate take }
        ...
        member _.GetCountBySpaceAsync(spaceId: SpaceId) : Task<int> =
            task { return resourceList |> List.filter (fun r -> r.State.SpaceId = spaceId) |> List.length }
        ...
        member _.DeleteAsync(resource: ValidatedResource) : Task<Result<unit, DomainError>> =
            task {
        ...
        member _.ExistsByNameAsync(resourceName: ResourceName) : Task<bool> =
            task { return resourceList |> List.exists (fun r -> r.State.Name = resourceName) }
...
[<Fact>]
let ``GetAllResources returns only resources from specified space`` () =
    task {
        // Arrange
...
[<Fact>]
let ``GetAllResources with pagination returns correct subset`` () =
    task {
        // Arrange
        let space1 = SpaceId.NewId()
...
[<Fact>]
let ``GetAllResources returns empty list when no resources in space`` () =
    task {
        // Arrange
        let space1 = SpaceId.NewId()
        let space2 = SpaceId.NewId()
...
[<Fact>]
let ``CreateResource allows same name in different spaces`` () =
    task {
        // Arrange
        let space1 = SpaceId.NewId()
        let space2 = SpaceId.NewId()
...
[<Fact>]
let ``CreateResource rejects duplicate name in same space`` () =
    task {
        // Arrange
        let space1 = SpaceId.NewId()
...
[<Fact>]
let ``GetAllResources with negative skip returns validation error`` () =
    task {
        // Arrange
        let space1 = SpaceId.NewId()
...
[<Fact>]
let ``GetAllResources with take less than or equal to 0 returns validation error`` () =
    task {
        // Arrange
        let space1 = SpaceId.NewId()
        let resources = [ createTestResource "Resource 1" space1 ]
...
[<Fact>]
let ``GetAllResources with take greater than 100 returns validation error`` () =
    task {
        // Arrange
        let space1 = SpaceId.NewId()
        let resources = [ createTestResource "Resource 1" space1 ]
