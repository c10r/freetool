...
type MockAppRepository(apps: ValidatedApp list, deletedApps: ValidatedApp list) =
    ...
    interface IAppRepository with
        member _.GetByIdAsync(appId: AppId) : Task<ValidatedApp option> =
            task { return appList |> List.tryFind (fun a -> a.State.Id = appId) }
        ...
        member _.ExistsAsync(appId: AppId) =
            task { return appList |> List.exists (fun a -> a.State.Id = appId) }
        ...
        member _.GetDeletedByIdAsync(appId: AppId) : Task<ValidatedApp option> =
            task { return deletedList |> List.tryFind (fun a -> a.State.Id = appId) }
...
type MockFolderRepository(folders: ValidatedFolder list, deletedFolders: ValidatedFolder list) =
    ...
    interface IFolderRepository with
        member _.GetByIdAsync(folderId: FolderId) : Task<ValidatedFolder option> =
            task { return folderList |> List.tryFind (fun f -> f.State.Id = folderId) }

        member _.GetChildrenAsync(folderId: FolderId) : Task<ValidatedFolder list> =
            task { return folderList |> List.filter (fun f -> f.State.ParentId = Some folderId) }
        ...
        member _.UpdateAsync(folder: ValidatedFolder) : Task<Result<unit, DomainError>> =
            task {
                folderList <- folder :: (folderList |> List.filter (fun f -> f.State.Id <> folder.State.Id))
                return Ok()
            }

        member _.DeleteAsync(folder: ValidatedFolder) : Task<Result<unit, DomainError>> =
            task {
                folderList <- folderList |> List.filter (fun f -> f.State.Id <> folder.State.Id)
                return Ok()
            }

        member _.ExistsAsync(folderId: FolderId) =
            task { return folderList |> List.exists (fun f -> f.State.Id = folderId) }

        member _.ExistsByNameInParentAsync _ _ = task { return false }
        member _.GetCountAsync() = task { return folderList.Length }

        member _.GetCountBySpaceAsync(spaceId: SpaceId) =
            task { return folderList |> List.filter (fun f -> f.State.SpaceId = spaceId) |> List.length }

        member _.GetBySpaceIdsAsync (spaceIds: SpaceId list) (_skip: int) (_take: int) : Task<ValidatedFolder list> =
            task { return folderList |> List.filter (fun f -> spaceIds |> List.contains f.State.SpaceId) }
        ...
        member _.GetDeletedBySpaceAsync(spaceId: SpaceId) : Task<ValidatedFolder list> =
            task { return deletedList |> List.filter (fun f -> f.State.SpaceId = spaceId) }

        member _.GetDeletedByIdAsync(folderId: FolderId) : Task<ValidatedFolder option> =
            task { return deletedList |> List.tryFind (fun f -> f.State.Id = folderId) }
...
type MockResourceRepository(resources: ValidatedResource list, deletedResources: ValidatedResource list) =
    ...
    interface IResourceRepository with
        member _.GetByIdAsync(resourceId: ResourceId) : Task<ValidatedResource option> =
            task { return resourceList |> List.tryFind (fun r -> r.State.Id = resourceId) }
        ...
        member _.ExistsAsync(resourceId: ResourceId) =
            task { return resourceList |> List.exists (fun r -> r.State.Id = resourceId) }
        ...
        member _.GetDeletedBySpaceAsync(spaceId: SpaceId) : Task<ValidatedResource list> =
            task { return deletedList |> List.filter (fun r -> r.State.SpaceId = spaceId) }

        member _.GetDeletedByIdAsync(resourceId: ResourceId) : Task<ValidatedResource option> =
            task { return deletedList |> List.tryFind (fun r -> r.State.Id = resourceId) }
...
[<Fact>]
let ``GetTrashBySpace returns deleted items`` () =
    task {
        // Arrange
...
[<Fact>]
let ``GetTrashBySpace returns empty for space with no trash`` () =
    task {
        // Arrange
        let spaceId = SpaceId.NewId()
...
[<Fact>]
let ``GetTrashBySpace paginates results correctly`` () =
    task {
        // Arrange
        let spaceId = SpaceId.NewId()
...
[<Fact>]
let ``GetTrashBySpace returns invalid space ID error`` () =
    task {
        // Arrange
        let appRepository = MockAppRepository([], []) :> IAppRepository
        let folderRepository = MockFolderRepository([], []) :> IFolderRepository
...
[<Fact>]
let ``RestoreApp restores deleted app`` () =
    task {
        // Arrange
        let spaceId = SpaceId.NewId()
        let folderId = FolderId.NewId()
...
[<Fact>]
let ``RestoreApp returns NotFound for nonexistent app`` () =
    task {
        // Arrange
        let appRepository = MockAppRepository([], []) :> IAppRepository
        let folderRepository = MockFolderRepository([], []) :> IFolderRepository
...
[<Fact>]
let ``RestoreApp renames on conflict`` () =
    task {
        // Arrange
        let spaceId = SpaceId.NewId()
        let folderId = FolderId.NewId()
...
[<Fact>]
let ``RestoreApp auto-restores deleted resource`` () =
    task {
        // Arrange
        let spaceId = SpaceId.NewId()
        let folderId = FolderId.NewId()
...
[<Fact>]
let ``RestoreFolder restores folder and children`` () =
    task {
        // Arrange
        let spaceId = SpaceId.NewId()
        let deletedFolder = createDeletedFolder "Deleted Folder" spaceId None
...
[<Fact>]
let ``RestoreFolder renames on conflict`` () =
    task {
        // Arrange
        let spaceId = SpaceId.NewId()
...
[<Fact>]
let ``RestoreResource restores deleted resource`` () =
    task {
        // Arrange
        let spaceId = SpaceId.NewId()
        let deletedResource = createDeletedResource "Deleted Resource" spaceId
