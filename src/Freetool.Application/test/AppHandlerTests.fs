...
type MockAppRepository(apps: ValidatedApp list) =
    ...
    interface IAppRepository with
        ...
        member _.GetAllAsync (skip: int) (take: int) : Task<ValidatedApp list> =
            task { return appList |> List.skip skip |> List.truncate take }
        ...
        member _.DeleteAsync (appId: AppId) (_actorUserId: UserId) : Task<Result<unit, DomainError>> =
            task {
        ...
        member _.GetCountByFolderIdAsync(folderId: FolderId) : Task<int> =
            task { return appList |> List.filter (fun a -> a.State.FolderId = folderId) |> List.length }
...
[<Fact>]
let ``CreateApp succeeds with valid data`` () =
    task {
        // Arrange
...
[<Fact>]
let ``CreateApp fails when app name exists in folder`` () =
    task {
        // Arrange
        let folderId = FolderId.NewId()
        let spaceId = SpaceId.NewId()
...
[<Fact>]
let ``DeleteApp removes app from repository`` () =
    task {
        // Arrange
        let folderId = FolderId.NewId()
        let spaceId = SpaceId.NewId()
...
[<Fact>]
let ``DeleteApp returns NotFound for nonexistent app`` () =
    task {
        // Arrange
        let actorUserId = UserId.NewId()
...
[<Fact>]
let ``GetAppById returns app when exists`` () =
    task {
        // Arrange
        let folderId = FolderId.NewId()
        let spaceId = SpaceId.NewId()
...
[<Fact>]
let ``GetAppById returns NotFound for nonexistent app`` () =
    task {
        // Arrange
        let nonExistentAppId = Guid.NewGuid().ToString()
...
[<Fact>]
let ``GetAppsByFolderId returns apps in folder with pagination`` () =
    task {
        // Arrange
        let folderId = FolderId.NewId()
        let spaceId = SpaceId.NewId()
...
[<Fact>]
let ``GetAppsByFolderId with negative skip returns ValidationError`` () =
    task {
        // Arrange
        let folderId = FolderId.NewId()
...
[<Fact>]
let ``GetAppsByFolderId with take greater than 100 returns ValidationError`` () =
    task {
        // Arrange
        let folderId = FolderId.NewId()
...
[<Fact>]
let ``GetAllApps returns paginated results`` () =
    task {
        // Arrange
...
[<Fact>]
let ``GetAppsBySpaceIds returns apps from multiple spaces`` () =
    task {
        // Arrange
        let folderId = FolderId.NewId()
...
[<Fact>]
let ``GetAppsBySpaceIds with empty list returns empty result`` () =
    task {
        // Arrange
        let folderId = FolderId.NewId()
        let spaceId = SpaceId.NewId()
...
[<Fact>]
let ``UpdateAppName changes name and creates event`` () =
    task {
        // Arrange
        let folderId = FolderId.NewId()
        let spaceId = SpaceId.NewId()
