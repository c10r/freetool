...
type MockFolderRepository(folders: ValidatedFolder list) =
    ...
    interface IFolderRepository with
        ...
        member _.GetRootFoldersAsync (skip: int) (take: int) : Task<ValidatedFolder list> =
            task {
                return
                    folderList
        ...
        member _.ExistsAsync(folderId: FolderId) : Task<bool> =
            task { return folderList |> List.exists (fun f -> f.State.Id = folderId) }

        member _.ExistsByNameInParentAsync (folderName: FolderName) (parentId: FolderId option) : Task<bool> =
            task {
                return
                    folderList
